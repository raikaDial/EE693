...
void ECG_Filter::pan85_filter(float* signal, uint16_t length) {
    if(length > 32) {
        ...
        // Remove edge effects by chopping off first 32
...
void ECG_Filter::pan85_filter(int32_t* signal, uint16_t length) {
    if(length > 32) {
        pan85_lp(signal,length);
        pan85_hp(signal,length);
        pan85_dev(signal,length);
...
void ECG_Filter::pan85_lp(int32_t* signal, uint16_t length) {
...
void ECG_Filter::pan85_hp(int32_t* signal, uint16_t length) {
    int32_t* output = new int32_t[length];
...
void ECG_Filter::pan85_dev(int32_t* signal, uint16_t length) {
    int32_t* output = new int32_t[length];
...
//void ECG_Filter::square_signal(int32_t* signal, uint16_t length) {
//    for(int i=0; i<length; ++i)
//        signal[i] = signal[i]*signal[i];
//}
//
//void ECG_Filter::copy_signal(int32_t* source, int32_t* dest, uint16_t length) {
//    for(int i=0; i<length; ++i)
//        dest[i] = source[i];
//}
