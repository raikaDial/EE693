...
void i2c0_isr(void) // I2C0 ISR
{
    ...
    i2c_isr_handler(&(i2c_t3::i2cData[0]),0);
        ...
        if( i2c_t3::done_(&(i2c_t3::i2cData[0]))
            && (i2c_t3::i2cData[0].rxBufferLength - i2c_t3::i2cData[0].rxBufferIndex == 4)
            && Wire.spo2_read
        ) {
            //digitalWriteFast(22, HIGH);
            //Serial.printf("%d bytes available from i2c\n", Wire.available());
            for(int i=0; i<4; ++i){
                Wire.spo2_bytes[i] = Wire.readByte();
            }
            Wire.spo2_flag = 1;
            Wire.spo2_read = 0;
